...
ResultSet rs=null;
if(srt.equals("least"))
rs=st.executeQuery("select videoid,url,name,description,nolikes from talent where category='news' and isapproved='yes' order by right(videoid,13)");
else if(srt.equals("most"))
rs=st.executeQuery("select videoid,url,name,description,nolikes from talent where category='news' and isapproved='yes' order by right(videoid,13) desc");
else if(srt.equals("likes"))
rs=st.executeQuery("select videoid,url,name,description,nolikes from talent where category='news' and isapproved='yes' order by nolikes desc");
while(rs.next()){
